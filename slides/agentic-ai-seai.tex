...
% SLIDE 30 — Hallucinations in Agentic Systems
...
% SLIDE 31 — Common Agent Frameworks in Practice
...
% SLIDE 32 — Persona vs Task-Oriented Agents
...
% SLIDE 33 — Practice: LangChain Tool Calling
\begin{frame}[fragile]{Practice: LangChain Tool Calling (Minimal)}
\small
\begin{lstlisting}
...
response = agent.invoke("What is 19 + 23? Use the tool.")
print(response)
\end{lstlisting}

\textit{Key idea:} Tool integration becomes a reusable pattern, not ad-hoc glue code.
\end{frame}

% SLIDE 34 — Practice: LangGraph State Machine
...
g.add_conditional_edges("step", should_stop, {
    "step": "step",
    END: END
})
...
\textit{Key idea:} Control-flow is explicit (graphs), not hidden inside prompts.
\end{frame}

% SLIDE 35 — Practice: LangChain vs LangGraph
...
% SLIDE 36 — Protocols: MCP vs A2A (Overview)
...
% SLIDE 37 — MCP: Client–Host–Server architecture
...
% SLIDE 38 — A2A: Agent-to-Agent protocol
...
% SLIDE 39 — MCP vs A2A vs Frameworks
...
% SLIDE 40 — When to use (or not use) MCP/A2A
