...
\subtitle{A Stateful Multi-Stage Agentic Workflow}
...
\begin{tikzpicture}[
    ...
    \draw[->] (critic.west) to[out=180,in=180] node[left] {rejected + retries left} (planner.west);
...
\begin{enumerate}
    ...
    \item \textbf{CONFIDENCE} --- computes reliability from grounded places (\(70\%\)) + flight signal (\(30\%\), when available); if low ($<0.7$), triggers HITL.
...
\begin{itemize}
    \item Input: \texttt{user\_input}, \texttt{destination}, \texttt{interests}, \texttt{budget}, \texttt{budget\_total}, \texttt{companion}, \texttt{origin}, \texttt{depart\_date}, \texttt{return\_date}
    ...
    \item Flight output: \texttt{flight\_options}, \texttt{flight\_summary}, \texttt{flight\_confidence\_score}
    \item Control fields: \texttt{confidence\_score}, \texttt{critic\_feedback}, \texttt{retry\_count}, \texttt{is\_approved}
    \item Context fields: \texttt{banned\_places}, \texttt{budget\_context}
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Robust tool interface}: tools return \texttt{list | error\_string}; nodes apply safe fallback on errors (no crashes).
    \item \textbf{Confidence signal}: weighted blend of grounded-place reliability (\(70\%\)) and flight confidence (\(30\%\), if present).
\end{itemize}
...
\begin{itemize}
    \item If blended \texttt{confidence\_score < 0.7},
...
\textbf{Key implementation lessons}
\begin{itemize}
    \item \textbf{Deterministic gates beat self-reported confidence:} grounding-driven confidence (places-first, flights as secondary signal) made routing more stable.
    \item \textbf{Fail-soft tooling is mandatory:} tool errors must degrade gracefully (fallback + continue), not crash the graph.
    \item \textbf{Input normalization is essential:} city\(\to\)IATA mapping and date normalization need strict guards to avoid silent misrouting.
    \item \textbf{Retry loops need hard boundaries:} explicit retry caps and failure exits prevent planner-critic dead loops.
\end{itemize}
