...
\subtitle{A Comprehensive Guide to Engineering Autonomous Systems}
...
\begin{frame}{2. Anatomy of an Agent}
    ...
    \begin{enumerate}
        ...
        \item \textbf{Planning Module:} Decomposes high-level goals into sub-tasks (Chain of Thought).
...
\begin{frame}{7. RAG within Agents (Knowledge Augmentation)}
    ...
    When an agent fails, was it the Reasoning or the Retrieval?
    \begin{itemize}
        \item \textbf{Chunking Strategy:} Fixed size vs. Semantic chunking.
        \item \textbf{Retrieval Metric:} Recall@K (Did we find the right doc?).
        \item \textbf{Generation Metric:} Faithfulness (Did we hallucinate based on the doc?).
    \end{itemize}
    
    \textbf{Advanced RAG for Agents:}
    \begin{itemize}
        \item \textit{Self-Querying:} Agent converts "Sold houses in Seattle" into SQL/Metadata filters {\texttt{city='Seattle', status='sold'}}.
        \item \textit{Hybrid Search:} Keywords (BM25) + Vectors (Cosine Similarity).
    \end{itemize}
\end{frame}

% SLIDE 10
\begin{frame}{8. Architectural Anti-Patterns}
    \begin{columns}
        \column{0.5\textwidth}
...
% SLIDE 11
\begin{frame}{9. Orchestration: Single vs. Multi-Agent Systems (MAS)}
    \begin{table}[]
        \begin{tabular}{@{}lll@{}}
            \toprule
            ...
            Use Case & Chatbot, Search & Software Dev, Complex Supply Chain \\ \bottomrule
        \end{tabular}
    \end{table}
    
    \textbf{The Law of MAS:} Complexity grows quadratically with the number of agents due to communication overhead. Only use MAS when a single prompt cannot hold all necessary instructions/tools.
\end{frame}

% SLIDE 12
\begin{frame}{10. Pattern A: The Router (Gateway)}
...
% SLIDE 13
\begin{frame}{11. Pattern B: Hierarchical Teams (Boss-Worker)}
    \textbf{Structure:}
    \begin{itemize}
...
% SLIDE 14
\begin{frame}{12. Pattern C: Sequential Handoffs (The Chain)}
...
% SLIDE 15
\begin{frame}{13. Inter-Agent Communication Protocols}
    How do agents talk?
    
    \textbf{1. Shared Blackboard (Memory)}
    \begin{itemize}
        \item A single global state object readable/writable by all.
        \item \textit{Risk:} Race conditions, context pollution.
    \end{itemize}
...
% SLIDE 16
\begin{frame}{14. State Management in Orchestration}
...
% SLIDE 17
\begin{frame}{15. Resilience and Error Recovery}
...
% SLIDE 18
\begin{frame}{16. Observability: Seeing Inside the Agent's Mind}
...
% SLIDE 19
\begin{frame}{17. Pillar 1: Logging (The Diary)}
    ...
    \begin{itemize}
        ...
        \item \textbf{Chain of Thought:} Log the "scratchpad" reasoning separately from the final answer.
    \end{itemize}
...
% SLIDE 20
\begin{frame}{18. Pillar 2: Tracing (The Narrative)}
...
% SLIDE 21
\begin{frame}{19. Pillar 3: Metrics (The Scorecard)}
...
% SLIDE 22
\begin{frame}{20. Evaluation: The Outside-In Framework}
...
% SLIDE 23
\begin{frame}{21. Hierarchy of Eval: Black Box vs. Glass Box}
    ...
    \begin{itemize}
        ...
        \item \textit{Limit:} Doesn't explain *why* it failed.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Code Eval:} Did the generated code compile?
    \end{itemize}
    
    \textit{Strategy:} Start with Black Box. If score < Threshold, trigger Glass Box deep dive.
\end{frame}

% SLIDE 24
\begin{frame}{22. Automated Metrics (The Low Bar)}
...
% SLIDE 25
\begin{frame}{23. LLM-as-a-Judge (The Scalable Critic)}
...
% SLIDE 26
\begin{frame}{24. Agent-as-a-Judge (Process Evaluator)}
...
% SLIDE 27
\begin{frame}{25. Human-in-the-Loop (HITL) Evaluation}
...
% SLIDE 28
\begin{frame}{26. Safety \& Red Teaming}
    ...
    \begin{itemize}
        \item \textbf{Input Rail:} Scan user prompt for PII/Injection before sending to Agent.
        \item \textbf{Output Rail:} Scan tool outputs and final text for leakage.
        \item \textbf{Sandboxing:} Execute code tools in isolated Docker containers (e.g., E2B).
    \end{itemize}
\end{frame}

% SLIDE 29
\begin{frame}{27. The Agent Quality Flywheel}
...
% SLIDE 30
\begin{frame}{28. Creating a "Golden Dataset"}
...
% SLIDE 31
\begin{frame}{29. Deployment Strategies (Ops)}
    \textbf{Shadow Mode:}
...
% SLIDE 32 (Bonus/Buffer - counts as 30 in logic)
\begin{frame}{30. Future Trends}
    \begin{itemize}
...
% SLIDE 33 (Final - counts as 31)
\begin{frame}{31. Conclusion \& Key Takeaways}
    \begin{enumerate}
...
% SLIDE 34
\begin{frame}{32. Acknowledgements}
\centering
